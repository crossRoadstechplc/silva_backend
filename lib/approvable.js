...
async function approveLine(objectType, lineId, user, programId, _comment) {
  ...
  const now = new Date();
  const data = { status: "approved", approvedAt: now };
  if (objectType === "rate_card_line") {
  ...
  if (objectType === "rate_card_line" && line.supersedesId) {
    await delegate.updateMany({
      where: { id: line.supersedesId, programId, effectiveTo: null },
