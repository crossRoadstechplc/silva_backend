...
function serializeRateCardLineForFarmOwner(line, thresholdPct) {
  ...
  // Approval queue needs variance/flags/justification; approved catalogue stays lean.
  if (enriched.status === "submitted") {
    return {
      id: enriched.id,
      ...
      benchmarkFarmARate: enriched.benchmarkFarmARate,
      benchmarkFarmBRate: enriched.benchmarkFarmBRate,
      spxJustificationNote: enriched.spxJustificationNote,
      variancePct: enriched.variancePct,
  ...
  return {
    ...
    activity: line.activity
      ? {
          ...
          category: line.activity.template?.category ?? null,
        }
      : null,
    electionStatus: line.electionStatus,
    sequence: line.sequence,
