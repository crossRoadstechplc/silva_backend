...
/** Fee Schedule sheet: the 36-month fee projection below the elective lines. */
function parseFeeMonthlySchedule(chakaPath = CHAKA_PATH) {
  const rows = readSheet(chakaPath, "Fee Schedule");
  const headerIdx = rows.findIndex((r) => str(r[0]) === "Month #");
  if (headerIdx < 0) return [];

  const header = rows[headerIdx].map(str);
  const col = (name) => header.findIndex((h) => h.startsWith(name));
  const confirmedCol = col("Confirmed");
  const electiveCol = col("Elective Subtotal");
  const totalCol = col("Total Monthly Fee");
  const cumulativeCol = col("Cumulative Total");

  const months = [];
  for (let i = headerIdx + 1; i < rows.length; i++) {
    const r = rows[i];
    const monthIndex = num(r[0]);
    if (monthIndex == null) continue;
    months.push({
      monthIndex: Math.round(monthIndex),
      confirmedFeeEtb: num(r[confirmedCol]),
      electiveFeeEtb: num(r[electiveCol]),
      feeEtb: num(r[totalCol]),
      cumulativeFeeEtb: num(r[cumulativeCol]),
    });
  }
  return months;
}
